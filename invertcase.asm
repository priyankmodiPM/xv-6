...
main(int argc, char *argv[])
{
  ...
  2e:	83 c3 01             	add    $0x1,%ebx
    invertcase(argv[i]);
  31:	e8 2a 00 00 00       	call   60 <invertcase>
...
main(int argc, char *argv[])
{
  ...
  40:	68 84 07 00 00       	push   $0x784
  45:	6a 01                	push   $0x1
  47:	e8 14 04 00 00       	call   460 <printf>
  exit();
  4c:	e8 b1 02 00 00       	call   302 <exit>
  51:	66 90                	xchg   %ax,%ax
  53:	66 90                	xchg   %ax,%ax
  55:	66 90                	xchg   %ax,%ax
  57:	66 90                	xchg   %ax,%ax
...
void invertcase(char *c){
  ...
  6d:	89 da                	mov    %ebx,%edx
  6f:	84 c0                	test   %al,%al
  71:	75 14                	jne    87 <invertcase+0x27>
  73:	eb 32                	jmp    a7 <invertcase+0x47>
...
void invertcase(char *c){
  ...
  83:	84 c0                	test   %al,%al
  85:	74 20                	je     a7 <invertcase+0x47>
		if (c[i] >= 'A' && c[i] <= 'Z')
  87:	8d 48 bf             	lea    -0x41(%eax),%ecx
  8a:	80 f9 19             	cmp    $0x19,%cl
  8d:	76 e9                	jbe    78 <invertcase+0x18>
	    	c[i] = c[i] + 32;
	    else if (c[i] >= 'a' && c[i] <= 'z')
  8f:	8d 48 9f             	lea    -0x61(%eax),%ecx
  92:	80 f9 19             	cmp    $0x19,%cl
  95:	77 e6                	ja     7d <invertcase+0x1d>
	        c[i] = c[i] - 32;
  97:	83 e8 20             	sub    $0x20,%eax
#include "stat.h"
#include "user.h"
...
void invertcase(char *c){
	int i;
	for(i=0;c[i]!='\0';i++){
  a0:	0f b6 02             	movzbl (%edx),%eax
  a3:	84 c0                	test   %al,%al
  a5:	75 e0                	jne    87 <invertcase+0x27>
	    else if (c[i] >= 'a' && c[i] <= 'z')
	        c[i] = c[i] - 32;
	    else
	    	c[i] = c[i];  
  ...
  b0:	6a 01                	push   $0x1
  b2:	e8 a9 03 00 00       	call   460 <printf>
}
...
printint(int fd, int xx, int base, int sgn)
{
 ...
 3ff:	f7 f6                	div    %esi
 401:	0f b6 92 90 07 00 00 	movzbl 0x790(%edx),%edx
  }while((x /= base) != 0);
...
putc(int fd, char c)
{
 ...
 578:	89 45 d0             	mov    %eax,-0x30(%ebp)
        if(s == 0)
          s = "(null)";
 57b:	b8 86 07 00 00       	mov    $0x786,%eax
...
free(void *ap)
{
 ...
 657:	89 08                	mov    %ecx,(%eax)
  freep = p;
 659:	a3 58 0a 00 00       	mov    %eax,0xa58
}
 65e:	5b                   	pop    %ebx
 65f:	5e                   	pop    %esi
 660:	5f                   	pop    %edi
 ...
 67a:	03 53 fc             	add    -0x4(%ebx),%edx
    ...
    p->s.ptr = bp;
  freep = p;
 67d:	a3 58 0a 00 00       	mov    %eax,0xa58
    bp->s.size += p->s.ptr->s.size;
    bp->s.ptr = p->s.ptr->s.ptr;
  } else
    bp->s.ptr = p->s.ptr;
...
malloc(uint nbytes)
{
 ...
 73c:	89 78 04             	mov    %edi,0x4(%eax)
      ...
      freep = prevp;
 73f:	89 15 58 0a 00 00    	mov    %edx,0xa58
      return (void*)(p + 1);
 ...
 754:	eb e9                	jmp    73f <malloc+0xaf>
  ...
  if((prevp = freep) == 0){
    base.s.ptr = freep = prevp = &base;
 756:	c7 05 58 0a 00 00 5c 	movl   $0xa5c,0xa58
 75d:	0a 00 00 
 760:	c7 05 5c 0a 00 00 5c 	movl   $0xa5c,0xa5c
 767:	0a 00 00 
    base.s.size = 0;
 76a:	b8 5c 0a 00 00       	mov    $0xa5c,%eax
 76f:	c7 05 60 0a 00 00 00 	movl   $0x0,0xa60
 776:	00 00 00 
