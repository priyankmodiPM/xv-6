...
{   
   ...
   f:	83 ec 10             	sub    $0x10,%esp
    int start, end ;
    start = uptime();
  12:	e8 63 03 00 00       	call   37a <uptime>
  17:	89 c3                	mov    %eax,%ebx
    double d = 0.01, z, x, id ; 

    id =fork() ; 
  19:	e8 bc 02 00 00       	call   2da <fork>
  1e:	89 45 f4             	mov    %eax,-0xc(%ebp)
  21:	db 45 f4             	fildl  -0xc(%ebp)
    if(id < 0){
  24:	d9 ee                	fldz   
  26:	d9 c0                	fld    %st(0)
  28:	db ea                	fucomi %st(2),%st
  2a:	77 4e                	ja     7a <main+0x7a>
  2c:	d9 ca                	fxch   %st(2)
        printf(1,"%d failed in fork!\n", getpid()) ;
    } else if(id > 0){
  2e:	df ea                	fucomip %st(2),%st
  30:	dd d9                	fstp   %st(1)
  32:	76 28                	jbe    5c <main+0x5c>
  34:	dd d8                	fstp   %st(0)
        // printf(1, "Parent %d creating child %d\n", getpid(), id) ; 
        wait() ;
  36:	e8 af 02 00 00       	call   2ea <wait>
  ...
  3b:	e8 3a 03 00 00       	call   37a <uptime>
    printf(1,"Time taken for pid %d : %d\n", getpid(), end - start) ; 
  40:	29 d8                	sub    %ebx,%eax
  42:	89 c3                	mov    %eax,%ebx
  44:	e8 19 03 00 00       	call   362 <getpid>
  49:	53                   	push   %ebx
  4a:	50                   	push   %eax
  4b:	68 74 07 00 00       	push   $0x774
  50:	6a 01                	push   $0x1
  ...
  57:	e8 86 02 00 00       	call   2e2 <exit>
        ...
        for(z =0 ; z < 2000000.0 ; z+=d)
  5c:	dd 05 90 07 00 00    	fldl   0x790
  62:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
  68:	dc c1                	fadd   %st,%st(1)
  6a:	d9 05 98 07 00 00    	flds   0x798
  70:	df ea                	fucomip %st(2),%st
  72:	77 f4                	ja     68 <main+0x68>
  74:	dd d8                	fstp   %st(0)
  76:	dd d8                	fstp   %st(0)
  78:	eb c1                	jmp    3b <main+0x3b>
  7a:	dd d8                	fstp   %st(0)
  7c:	dd d8                	fstp   %st(0)
  7e:	dd d8                	fstp   %st(0)
    start = uptime();
    ...
    if(id < 0){
        printf(1,"%d failed in fork!\n", getpid()) ;
  80:	e8 dd 02 00 00       	call   362 <getpid>
  85:	52                   	push   %edx
  86:	50                   	push   %eax
  87:	68 60 07 00 00       	push   $0x760
  8c:	6a 01                	push   $0x1
  8e:	e8 ad 03 00 00       	call   440 <printf>
  93:	83 c4 10             	add    $0x10,%esp
  96:	eb a3                	jmp    3b <main+0x3b>
  98:	66 90                	xchg   %ax,%ax
  9a:	66 90                	xchg   %ax,%ax
  9c:	66 90                	xchg   %ax,%ax
  9e:	66 90                	xchg   %ax,%ax
...
printint(int fd, int xx, int base, int sgn)
{
 ...
 3df:	f7 f6                	div    %esi
 3e1:	0f b6 92 a4 07 00 00 	movzbl 0x7a4(%edx),%edx
  }while((x /= base) != 0);
...
putc(int fd, char c)
{
 ...
 558:	89 45 d0             	mov    %eax,-0x30(%ebp)
        if(s == 0)
          s = "(null)";
 55b:	b8 9c 07 00 00       	mov    $0x79c,%eax
